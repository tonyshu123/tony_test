...
void simcom_test_main() 
{
	...
	printf("hello tony,glade to see you hear,just for test");
        ...
        switch(opt)
        {
            ...
            case 99:
                exit(0);
            default:
                printf("error option,or fuction closed,pls enable compile in Makefile");
                break;
        }
    }
}
    ...
    while(1)
    {
        ...
        {  
            ...
            case 6:
                {
                    int ret_value;
    		        nas_get_sys_info_resp_msg_v01 sys_info;

                    ret_value = nas_get_sys_info(&sys_info);
                    
                    if (ret_value == (-1))
                    {
                        printf("get system info failure\n");
                        break;
                    }

                    /*cdma info*/
                    if (sys_info.cdma_sys_info_valid == 1)
                    {
                        printf("CDMA MCC = %c%c%c\n", sys_info.cdma_sys_info.cdma_specific_sys_info.network_id.mcc[0],
                                                        sys_info.cdma_sys_info.cdma_specific_sys_info.network_id.mcc[1],
                                                        sys_info.cdma_sys_info.cdma_specific_sys_info.network_id.mcc[2]);
                        printf("CDMA MNC = %c%c\n", sys_info.cdma_sys_info.cdma_specific_sys_info.network_id.mnc[0],
                                                        sys_info.cdma_sys_info.cdma_specific_sys_info.network_id.mnc[1]);
                        printf("CDMA SID = %d\n", sys_info.cdma_sys_info.cdma_specific_sys_info.cdma_sys_id.sid);
                        printf("CDMA NID = %d\n", sys_info.cdma_sys_info.cdma_specific_sys_info.cdma_sys_id.nid);
                    }

                    /*LTE info*/
                    if (sys_info.lte_sys_info_valid == 1)
                    {
                        printf("LTE MCC = %c%c%c\n", sys_info.lte_sys_info.threegpp_specific_sys_info.network_id.mcc[0],
                                                        sys_info.lte_sys_info.threegpp_specific_sys_info.network_id.mcc[1],
                                                        sys_info.lte_sys_info.threegpp_specific_sys_info.network_id.mcc[2]);
                        printf("LTE MNC = %c%c\n", sys_info.lte_sys_info.threegpp_specific_sys_info.network_id.mnc[0],
                                                    sys_info.lte_sys_info.threegpp_specific_sys_info.network_id.mnc[1]);
                        printf("LTE TAC = %d\n", sys_info.lte_sys_info.lte_specific_sys_info.tac);
                        printf("LTE CELLID = %d\n", sys_info.lte_sys_info.threegpp_specific_sys_info.cell_id);
                    }
            ...
            case 6:  //tyrlte
                {
                    ...
                    uint8_t urc_enable;
                    uint8_t try_flag;
                    printf("\nPlease input mode value(0:READ, 1:WRITE)\n");
                    memset(scan_string, 0, sizeof(scan_string));
                    if(fgets(scan_string, sizeof(scan_string), stdin) == NULL)
                        continue;
                    ...
                    if (0 == mode)  //read
                    {
                        simcom_network_trylte(mode, &urc_enable, &try_flag);
                        printf("urc_enable=%d, try_flag=%d\n", urc_enable, try_flag);
                    }else if (1 == mode)
                    {
                        printf("\nPlease input urc_enable(0:DISABLE, 1:ENABLE)\n");
                        memset(scan_string, 0, sizeof(scan_string));
                        if(fgets(scan_string, sizeof(scan_string), stdin) == NULL)
                            continue;
                        /* Convert the option to an integer, and switch on the option entered. */
                        urc_enable = atoi(scan_string);
                        if ((urc_enable !=1) && (urc_enable != 0))
                        {
                            printf("input urc_enable value err (%d)\n", urc_enable);
                            break;
                        }

                        printf("\nPlease input TRY_FLAG(0:not try lte, NULL: not set)\n");
                        memset(scan_string, 0, sizeof(scan_string));
                        if(fgets(scan_string, sizeof(scan_string), stdin) == NULL)
                            continue;
                        /* Convert the option to an integer, and switch on the option entered. */
                        try_flag = atoi(scan_string);
                        if (0 == try_flag)
                        {
                            simcom_network_trylte(mode, &urc_enable, &try_flag);
                        }else if (strstr(scan_string, "NULL") != NULL)
                        {
                            simcom_network_trylte(mode, &urc_enable, NULL);
                        }else
                        {
                            printf("input try_flag value err (%d)\n", try_flag);
                            break;
                        }

                    }else
                    {
                        printf("error mode value (%d)\n", mode);
                        break;
                    }
